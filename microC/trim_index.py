...
def trim(sample, fq1, fq2, index_seq):
    ...
    for read1, read2 in zip(
        ...
        insert_len = 0
        for i in range(len(index_seq)-7):
            sub_seq = index_seq[i:(i+6)]
            if sub_seq in seq1:
                insert_len = len(seq1.split(sub_seq)[0])
                break

        if len(seq1[:insert_len])<60 or len(seq2[:insert_len])<60:
            continue
...
if __name__ == "__main__":
    ...
    parser.add_argument("-i","--index", type=str, help="index sequence(must more than 10bp)")
